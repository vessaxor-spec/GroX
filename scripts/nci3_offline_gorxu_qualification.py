...
QUESTION = "Why does restore verification matter even when backups exist?"
...
def qualify(args: argparse.Namespace) -> None:
    ...
    scratch = layout.state_root / "nci3-llama-scratch"
    scratch.mkdir(parents=True, exist_ok=True)
    registry = ModelRegistry.from_asset_root(layout.asset_root, model_store_root=store.root)
    ...
    if runtime.active_models():
        raise AssertionError(f"model readiness auto-activated models: {runtime.active_models()}")
    ...
    missions_before = pilot.store.recent_missions(1000)
    crew_before = pilot.store.crew_states()
    direct = pilot.ask(QUESTION)
    if direct.get("status") != "answered":
        raise AssertionError(direct)
    if direct.get("commander_input") != QUESTION:
        raise AssertionError(direct)
    response = str(direct.get("response") or "")
    response_lower = response.lower()
    if "backup" not in response_lower or "restore" not in response_lower or len(response.strip()) < 40:
        raise AssertionError(f"direct local response was not useful for the bounded question: {response!r}")
    if direct.get("mission_created") is not False or direct.get("crew_delegated") is not False:
        raise AssertionError(direct)
    if direct.get("authority_changed") is not False:
        raise AssertionError(direct)
    if pilot.store.recent_missions(1000) != missions_before:
        raise AssertionError("direct assistance created a Mission")
    if pilot.store.crew_states() != crew_before:
        raise AssertionError("direct assistance changed Crew duty state")
    ...
    if not str(cognition.get("objective") or "").strip():
        raise AssertionError(f"local interpretation did not produce an objective: {cognition}")
    if not str(cognition.get("recommended_option") or "").strip():
        raise AssertionError(f"local planning did not produce a recommendation: {cognition}")
    ...
    if result.get("crew") not in candidate_crew:
        raise AssertionError(
            "deterministic routing did not select a locally recommended eligible Crew: "
    ...
    if not summary.startswith("GorXu:"):
        raise AssertionError(f"Pilot synthesis missing from qualifying Mission: {summary!r}")
    ...
    _dump(
        {
            "schema": "grox-nci3-offline-gorxu-qualification-v1",
            "status": "PASS",
            ...
            "mission": {
                ...
                "objective": cognition.get("objective"),
