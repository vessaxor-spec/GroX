...
def policy(**kwargs):
    base={
        ...
        'allow_order_text':True,
        'allow_craft':True,
        'allow_memory':True,
        'allowed_observation_actions':frozenset({'fs_list','fs_read','test_run'}),
    }
    base.update(kwargs)
...
class OpenAICrewCognitionProviderTests(unittest.TestCase):
    ...
    def test_disclosure_policy_restricts_context_action_schema_and_scope(self):
        captured={}
        restricted=OpenAICrewDisclosurePolicy(allowed_scopes=('README.md',))
        provider=OpenAICrewCognitionProvider(api_key='k',model='gpt-5.6-luna',disclosure_policy=restricted)
        ...
        with patch('grox.openai_crew_cognition.urlopen',side_effect=fake):
            provider.next_step(order=order,craft_context=[{'content':'SECRET-CRAFT'}],memory_context=[{'text':'SECRET-MEMORY'}],observations=[{'action':'fs_read','path':'README.md','content':'SECRET-FILE'}])
        text=captured['body']['input']
        for sentinel in ('SECRET-INTENT','SECRET-OBJECTIVE','SECRET-CRAFT','SECRET-MEMORY','SECRET-FILE'):
        ...
        with patch('grox.openai_crew_cognition.urlopen') as network:
            with self.assertRaisesRegex(CrewCognitionError,'scope exceeds'):
                provider.next_step(order={'scope':['docs']},craft_context=[],memory_context=[],observations=[])
            network.assert_not_called()

    def test_responses_request_is_structured_read_only_and_context_bounded(self):
        captured={}
        payload={'id':'resp_test_123','model':'gpt-5.6-luna-test','usage':{'input_tokens':120,'input_tokens_details':{'cached_tokens':30},'output_tokens':24,'output_tokens_details':{'reasoning_tokens':8},'total_tokens':144},'output':[{'type':'message','content':[{'type':'output_text','text':json.dumps({'action':'fs_read','path':'README.md','work_product':None})}]}]}
        def fake(request,timeout):
            captured['url']=request.full_url; captured['timeout']=timeout; captured['body']=json.loads(request.data.decode()); captured['authorization']=request.get_header('Authorization'); return _Response(payload)
        ...
        with patch('grox.openai_crew_cognition.urlopen',side_effect=fake):
            step=provider.next_step(order={'mission_id':'MSN-1','scope':['README.md'],'commander_intent':'Inspect README','objective':'inspect','allowed_actions':['fs_read']},craft_context=[{'heading':'Safety Boundaries','content':'read only'}],memory_context=[{'memory_id':'MEM-1','text':'prior evidence'}],observations=[{'action':'fs_list','path':'README.md','files':['README.md']}])
        ...
        body=captured['body']; self.assertFalse(body['store']); self.assertEqual(body['model'],'gpt-5.6-luna'); self.assertEqual(body['max_output_tokens'],700)
        self.assertEqual(body['text']['format']['schema']['properties']['action']['enum'],['finish','fs_list','fs_read','test_run'])
    ...
    def test_output_and_failures_are_bounded(self):
        provider=OpenAICrewCognitionProvider(api_key='k',model='gpt-5.6-luna',disclosure_policy=policy(allowed_observation_actions=frozenset()))
        order={'scope':['README.md']}
        with patch('grox.openai_crew_cognition.urlopen',return_value=_Response({'id':'r','output_text':json.dumps({'action':'finish','path':None,'work_product':'bounded result'})})):
            self.assertEqual(provider.next_step(order=order,craft_context=[],memory_context=[],observations=[])['action'],'finish')
