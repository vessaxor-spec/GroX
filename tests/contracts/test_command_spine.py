...
class SpineTest(unittest.TestCase):
    ...
    def test_reconstitution_purges_stale_crew_operational_state(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            store = StateStore(root / 'state.sqlite3')
            try:
                store.ensure_crew('systems-architect')
                store.db.execute("UPDATE crew_state SET status='retired' WHERE crew_id='systems-architect'")
                store.remember(
                    kind='semantic',
                    scope='crew',
                ...
                store.db.execute(
                    ...
                    ('ORD-audit', 'MSN-audit', 'systems-architect', 'inspect', 'completed', '{}', 't', 't'),
                )
                store.db.commit()
                ...
                self.assertEqual(
                    store.db.execute("SELECT COUNT(*) FROM crew_state WHERE crew_id='systems-architect'").fetchone()[0],
                    0,
                )
                self.assertEqual(
                    store.db.execute("SELECT COUNT(*) FROM memories WHERE crew_id='systems-architect'").fetchone()[0],
                    0,
                )
                self.assertEqual(
                    store.db.execute("SELECT COUNT(*) FROM crew_performance WHERE crew_id='systems-architect'").fetchone()[0],
                    0,
                )
                self.assertEqual(
