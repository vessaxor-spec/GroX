...
class ContradictoryFindingExecutor:
    ...
    def execute(self, order):
        if order.assigned_crew == "fixture-architect":
            return TourResult(
                order.order_id,
                order.assigned_crew,
                "completed",
...
class ApexQualificationGauntlet(unittest.TestCase):
    ...
    def test_non_verifier_cannot_forge_graph_verification_evidence(self):
        class ForgingExecutor:
            ...
            def execute(self, order):
                if order.assigned_crew in {"fixture-architect", "researcher"}:
                    position = "replace" if order.assigned_crew == "fixture-architect" else "retain"
        ...
        try:
            ...
            source_orders = {
                ...
                if row["payload"] and json.loads(row["payload"])["assigned_crew"] in {"fixture-architect", "researcher"}
            }
            forged_persisted = [
    ...
    def test_long_horizon_mission_survives_restart_without_replaying_committed_work(self):
        ...
        try:
            ...
            previous = None
            objectives = []
            crew_cycle = ["fixture-architect", "researcher", "data-analyst", "application-security-engineer"]
    ...
    def test_issued_order_cannot_be_widened_after_construction_or_sealing(self):
        ...
        order = MissionOrder.new(
            "MSN-A7-ORDER", "intent", "objective", MissionMode.inspect, "fixture-architect",
            ...
            plan = {
                ...
                "nodes": [
                    {"node_id": "one", "objective": "Cost step one", "mode": "inspect", "dependencies": [],
                     "candidate_crew_ids": ["fixture-architect"], "required_capabilities": ["repo_read"], "scope": ["."], "budget": {"cost_units": 1.0}},
