...
**Release baseline:** `v0.7.0` at `main@71ffd60769d81b5b249dac4eca56333ff27e26d0`
...
Next critical stage after A3 was A4 Executive Exception Loop and Durable Operations.

### A4 live qualification: PASSED

A4 adds a private `DurableState` operational ledger and a deterministic Executive Exception Loop under GorXu. Mission Graph plans, checkpoints, exception decisions, bounded resume/cancellation state, and supported Repair mutation journals survive process restart without creating another command layer. Ordinary recoverable failures require a real bounded read-only Crew consultation before replan; critical, irreversible, authority-divergent, or material-intent conditions reserve decision authority for the Commander.

Qualification Mission: `MSN-a62e95886c0a`.

Evidence:
- fresh GitHub-hosted Ubuntu 24.04 / Python 3.12 qualification passed **55/55** tests;
- the private A3 checkpoint re-verified before A4 activation;
- the exact independently qualified A4 branch source was materialized on the active flight computer;
- private SQLite migrated cleanly with graph-run, checkpoint, exception-decision, and mutation-journal tables while integrity remained `ok`;
- live qualification interrupted the Mission after `architect` had committed its node and while `researcher` was in flight;
- a fresh Pilot reopened the same Mission ID, marked in-flight state interrupted, and did not replay the committed architecture Order;
- two later injected `crew_unavailable` exceptions were independently persisted, compared, consulted through real read-only Crew Orders, and replanned within budget;
- no Commander escalation occurred for those ordinary reversible failures;
- the resumed Mission completed after one resume and two replans with independent graph verification PASS;
- cancellation, critical escalation, timeout normalization, exact Repair rollback, idempotent Repair replay, and fail-closed external divergence are covered by qualification tests.
