...
The original Stage 1 qualification above remains historical **12/12 KILLED** evidence. Protected source later extended the same permanent harness with twelve additional high-consequence Live Environment Awareness authority/evidence mutations. The current matrix is therefore **24/24 KILLED**; this extension does not rewrite the original Stage 1 run.
...
| 24 | Configured credential-alias availability must remain bound to the exact configured alias | replace the exact bound alias passed to `SecretAliasAwareness.inspect` in `src/grox/configured_credential_availability.py` with `fallback-alias` | `ConfiguredCredentialAliasAvailabilityTests.test_other_broker_alias_never_satisfies_missing_exact_configured_alias` | KILLED |

Latest exact-head evidence is PR #161 CI #555 / `32821385793`: Python 3.12 job `97720097426` killed **24/24** critical mutations with zero survivors and `source_restored_clean=true` while all health, reconstitution, operational-drift, source-provenance, and Post-Apex gates remained green. The #16 mutation remains `cognition-transport-presealed-authority`; #17 is `cognition-transport-origin-binding`; #18 is `cognition-endpoint-exact-binding`; #19 is `configured-cognition-discovery-state-separation`; #20 is `configured-connection-exact-resource-binding`; #21 is `configured-local-readiness-authorization-separation`; #22 is `secret-alias-exact-binding`; #23 is `configured-credential-binding-exact-resource`; #24 is `configured-credential-availability-exact-alias`. The original Stage 1 **12/12** qualification remains preserved above as historical evidence.
